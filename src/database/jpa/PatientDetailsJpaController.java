...
public class PatientDetailsJpaController implements Serializable {
	...
	public void create(PatientDetails patientDetails)
		...
		if (accountDetailsOrphanCheck != null) {
			...
			if (oldPatientDetailsOfAccountDetails != null) {
				...
				illegalOrphanMessages.add("The AccountDetails " + accountDetailsOrphanCheck
						+ " already has an item of type PatientDetails whose accountDetails column cannot be null. Please make another selection for the accountDetails field.");
			}
		}
		if (illegalOrphanMessages != null) {
	...
	public void edit(PatientDetails patientDetails)
		...
		try {
			...
			if (billDetailsOld != null && !billDetailsOld.equals(billDetailsNew)) {
				...
				illegalOrphanMessages.add("You must retain BillDetails " + billDetailsOld
						+ " since its patientId field is not nullable.");
			}
			if (accountDetailsNew != null && !accountDetailsNew.equals(accountDetailsOld)) {
				...
				if (oldPatientDetailsOfAccountDetails != null) {
					...
					illegalOrphanMessages.add("The AccountDetails " + accountDetailsNew
							+ " already has an item of type PatientDetails whose accountDetails column cannot be null. Please make another selection for the accountDetails field.");
				}
			}
			if (illegalOrphanMessages != null) {
	...
	public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException {
		...
		try {
			...
			if (billDetailsOrphanCheck != null) {
				...
				illegalOrphanMessages.add("This PatientDetails (" + patientDetails
						+ ") cannot be destroyed since the BillDetails " + billDetailsOrphanCheck
						+ " in its billDetails field has a non-nullable patientId field.");
			}
			if (illegalOrphanMessages != null) {
				throw new IllegalOrphanException(illegalOrphanMessages);
			}
	...
	public List<PatientDetails> findPatientDetailsByDoctorid(Integer id) {
		EntityManager em = getEntityManager();
		try {
			TypedQuery<PatientDetails> query = em.createNamedQuery("PatientDetails.findByDoctorId",
					PatientDetails.class);
			return query.setParameter("doctorId", id).getResultList();
		} catch (Exception e) {
			return (List<PatientDetails>) new PatientDetails();
		} finally {
			em.close();
		}
	}
